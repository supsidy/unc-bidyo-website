              ...
              >
                ...
                <div className="relative">
                  ...
                  {/* PNG cutout — NOT clipped, layered above the backdrop, free to overflow on hover.
                      ...
                      Two images are stacked so the hover swap crossfades instead of popping —
                      the default pose fades out while the hover pose fades in, both scaling up
                      together. If a member has no hoverImage, the second layer just mirrors the
                      first, so nothing visually changes on hover. */}
                  <img
                    src={member.image}
                    alt={member.name}
                    className="pointer-events-none absolute inset-x-0 bottom-0 z-20 w-full scale-125 origin-bottom object-contain object-bottom transition-all duration-500 ease-out group-hover:z-30 group-hover:scale-150 group-hover:opacity-0"
                  />
                  <img
                    src={member.hoverImage || member.image}
                    alt=""
                    aria-hidden="true"
                    className="pointer-events-none absolute inset-x-0 bottom-0 z-20 w-full scale-125 origin-bottom object-contain object-bottom opacity-0 transition-all duration-500 ease-out group-hover:z-30 group-hover:scale-150 group-hover:opacity-100"
                  />
                </div>
